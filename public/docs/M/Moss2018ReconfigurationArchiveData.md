...
- **Practice cross-check:** This is directly relevant to S3 if I move across multiple series, actors, and document types in DDR, but it must be balanced by close reading so that “mining” does not erase contested design knowledge into pattern alone. TODO (needs exact S3 workflow cross-link).
...
- **Practice cross-check:** This aligns closely with your gender-gap thread, with Grigg on finding aids, with Bowker and Star on classification, and with your need to surface overlooked contributors such as Patterson rather than reproduce inherited visibility hierarchies. TODO (needs exact gender-gap / metadata cross-link).
...
- **Practice cross-check:** This connects directly to Marciano on CAS, Jaillant and Aske on access and preprocessing, Boyd Davis/Vane/Kräutli on interrogability, and your oral-history/digital activation threads, but it needs grounding in what the DDR archive can actually sustain. TODO (needs exact methods cross-link).
...
- **Datafication of the humanities:** the treatment of evidence in many formats as data to be mined, filtered, linked, and transformed; I will use this term where archival traces become computational objects. `[@Moss2018ReconfigurationArchiveData, pp. 131–134]` :contentReference[oaicite:9]{index=9}
- **Macroscope / distant reading:** a mode of making sense of very large corpora through analytical tools rather than close reading of individual texts; I will use this as a useful but partial frame for archive activation. `[@Moss2018ReconfigurationArchiveData, pp. 128–131]` :contentReference[oaicite:10]{index=10}
- **Recursive trans-disciplinary curatorial conversation:** the paper’s formulation for collaboration among archivists, users, and engineers in shaping digital archives and tools; I will use this as a strong process term for S3 method design. `[@Moss2018ReconfigurationArchiveData, pp. 146–149]` :contentReference[oaicite:11]{index=11}
- **Demand-side archival practice:** an approach attentive to user communities, analytical tools, and changing ways of reading archives; I will use this term when justifying design choices around retrieval, interface, and computational modelling. `[@Moss2018ReconfigurationArchiveData, pp. 136–143]` :contentReference[oaicite:12]{index=12}
...
- **Link to my practice evidence (one concrete cross-reference):** Use alongside your S3 workflow notes, especially where archive records move into metadata structures, clustering, visual analytics, or provenance-aware retrieval. TODO (needs exact cross-reference).
